...
| `implemented_failing` | Development-only investigation test exposes an unresolved finding; it cannot support a parity or release claim |
| `implemented_with_xfail` | Development-only finding remains executable; the affected surface cannot be reported as parity-covered |
...
| Finding regressions | Preserve every discovered difference as an executable case until it is fixed, then lock the fix |
...
| Error tests | Ensure invalid input and separately declared out-of-scope capabilities return structured errors |
...
| Support matrix | `SUPPORT_MATRIX` connects `FeatureSpec` entries to registered operations | implemented | Generated by `molecule_ops!`; registry tests require one support and invariant entry per operation |
| Capability boundaries | Separately declared out-of-scope capabilities return structured errors | partial | This fail-closed rule prevents placeholder chemistry; it is not a disposition for mismatches inside supported features |
...
| Weak topology ops | Chemical graph state changes invalidate affected caches and preserve stable indices | partial | `sanitize` and `kekulize` are explicitly registered as weak operations; a unified profile-driven invariant runner is still incomplete |
...
| No silent chemistry guessing | Separately scoped capabilities return errors; supported-path differences are fixed | partial | A failing input inside a supported boundary cannot be relabeled unsupported |
...
| Parallelism | Parallel batch behavior is deterministic and order-preserving | implemented | ChEMBL 37 scalar/batch comparisons cover one and eight jobs; shared-object concurrent reads are repeated separately |
| High-feasibility molecular descriptors | Connectivity Chi, Hall-Kier/Kappa/Phi, selected Lipinski/ring/stereo counts, MQN, Labute ASA, and SlogP/SMR VSA reproduce the pinned source through one core per family, including cache lifecycle and contribution/vector ordering | implemented | Focused branch fixtures, the complete maintained 5,000-row profile (1,588,749 exact observations), and the complete ChEMBL 37 profile (903,237,331 exact observations over 2,897,804 mutually parseable records) report zero mismatch; architecture tests enforce sole path/Rb0/ring/MQN/Labute/VSA owners and borrowed Python cache ownership |
| Topological Torsion fingerprints | Four modern vectors, provenance, live options/JSON, dynamic unfolded widths, legacy adapters, explicit-valence cache state, edge graphs, errors, scalar/bulk equality, ordered batch execution, and source immutability match the pinned source boundary | implemented | `rdkit_topological_torsion_fingerprint_parity.rs` consumes all 152 focused rows across 12 profiles and all 5,000 maintained rows across nine profiles exactly; the 5,000-row matrix is a required CI gate. The complete ChEMBL 37 phase consumes all 2,897,819 source rows and records 127,503,376 exact vector/provenance comparisons with zero mismatch. Focused regressions cover query/dative/hypervalent/explicit-H graphs, direct AddHs composition, and source-defined live-option states; `rdkit_topological_torsion_fingerprint_batch.rs` covers indexed errors and deterministic 1/2/4-thread scalar equality without a second fingerprint core. |
...
| InChI scalar APIs | Four public scalar APIs have exact source-defined official-C/RDKit and Python tests plus structured diagnostics | implemented | `INCHI_API_PARITY_MATRIX` is the structured non-operation parity source. Separately named upstream APIs outside these four are capability boundaries, not accepted differences inside the supported surface. The official-C undefined `NormalizeAndCompare` allocation path maps to deterministic `allocation_failed` and is not an exact-parity claim. |
...
Every newly discovered mismatch should remain executable and visible until it
is fixed, and it must block a parity claim while open.
Every separately declared out-of-scope capability should return a structured
error instead of fabricated output.
