...
INVENTORY_PATH = REPO_ROOT / "tests/fixtures/conformer_generation/rdkit_inventory.jsonl"
...
def load_fixture_inventory() -> dict[str, Path]:
    ...
    for line in INVENTORY_PATH.read_text(encoding="utf-8").splitlines():
        ...
        if fixture is None:
            continue
        mapping[record["source"]] = REPO_ROOT / "tests/fixtures/conformer_generation" / fixture
    return mapping
...
def main() -> None:
    ...
    parser.add_argument(
        "--output",
        type=Path,
        default=Path("tests/golden/conformer_generation.jsonl"),
        help="output JSONL path (default: tests/golden/conformer_generation.jsonl)",
    )
    parser.add_argument(
        "--input",
        type=Path,
