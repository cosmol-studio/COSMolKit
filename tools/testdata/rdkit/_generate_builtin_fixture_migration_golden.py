...
DEFAULT_FIXTURE_ROOT = REPO_ROOT / "testdata" / "rdkit_builtin" / "fixtures"
FIXTURE_METADATA_NAMES = frozenset({"README.md", "source_manifest.jsonl"})
...
def build_records(fixture_root: Path) -> list[dict[str, object]]:
    ...
    for path in sorted(fixture_root.rglob("*")):
        if not path.is_file():
            continue
        if path.name in FIXTURE_METADATA_NAMES:
            continue
...
def main() -> None:
    ...
    parser.add_argument(
        ...
        help=(
            "fixture root. When invoked by generate_all.py this may be "
