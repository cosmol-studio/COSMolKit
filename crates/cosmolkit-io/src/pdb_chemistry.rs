//! RDKit-compatible chemistry postprocessing for detached PDB topology.
//!
//! This module is the single behavior owner for proximity bonding, standard
//! residue bond orders, basic PDB cleanup, and the post-stereo residue
//! chirality filter. Format parsing and runtime sanitize/hydrogen policy remain
//! in their respective boundary layers.

use cosmolkit_model::{
    AdjacencyList, Atom, AtomId, Bond, BondId, BondSpec, ChiralTag, CoordinateBlock, TopologyBlock,
};
...
pub struct PdbPostprocessParams {
    /// Run RDKit's hash-grid proximity bonding with H-H contacts disabled.
    pub proximity_bonding: bool,
    /// PDB parser flavor bits. Bit 8 requests standard-residue bond orders even
    /// when proximity bonding is disabled.
    pub flavor: u32,
}

struct DetachedPdbEditor<'a> {
    topology: &'a mut TopologyBlock,
    coordinates: &'a CoordinateBlock,
    adjacency: Vec<Vec<BondId>>,
}

impl<'a> DetachedPdbEditor<'a> {
    fn new(topology: &'a mut TopologyBlock, coordinates: &'a CoordinateBlock) -> Self {
        let mut adjacency = vec![Vec::new(); topology.atoms.len()];
        for bond in &topology.bonds {
            adjacency[bond.begin().index()].push(bond.id());
            adjacency[bond.end().index()].push(bond.id());
        }
        Self {
            topology,
            coordinates,
    ...
    fn bond(&self, bond: BondId) -> Option<&Bond> {
        self.topology.bonds.get(bond.index())
    }
    ...
    fn bond_between_atoms(&self, begin: AtomId, end: AtomId) -> Option<BondId> {
        self.neighbor_bonds(begin).iter().copied().find(|bond_id| {
            let bond = &self.topology.bonds[bond_id.index()];
            (bond.begin() == begin && bond.end() == end)
                || (bond.begin() == end && bond.end() == begin)
        })
    }

    fn add_bond(&mut self, spec: BondSpec) -> Result<BondId, PdbPostprocessError> {
        let begin = spec.begin();
        let end = spec.end();
        if begin.index() >= self.topology.atoms.len()
            || end.index() >= self.topology.atoms.len()
            || begin == end
        {
            return Err(PdbPostprocessError::InvalidState(format!(
                "cannot add bond between atom {} and atom {}",
                begin.index(),
                end.index()
            )));
        }
        let id = BondId::new(self.topology.bonds.len());
        self.topology.bonds.push(Bond::from_spec(id, spec));
    ...
    fn set_bond_order(
        ...
        let Some(bond) = self.topology.bonds.get_mut(bond.index()) else {
            return Err(PdbPostprocessError::InvalidState(format!(
                "bond {} disappeared while setting its order",
                bond.index()
            )));
        };
    ...
    fn remove_bond_between_atoms(&mut self, begin: AtomId, end: AtomId) -> Option<BondId> {
        let bond = self.bond_between_atoms(begin, end)?;
        self.topology.bonds.remove(bond.index());
        for (index, bond) in self.topology.bonds.iter_mut().enumerate() {
    ...
    fn rebuild_working_adjacency(&mut self) {
        self.adjacency = vec![Vec::new(); self.topology.atoms.len()];
    ...
    fn finish(self) {
        self.topology.adjacency =
            AdjacencyList::from_topology(self.topology.atoms.len(), &self.topology.bonds);
    }
}

fn is_blacklisted_pair_like_rdkit(
    editor: &DetachedPdbEditor<'_>,
    begin: AtomId,
    end: AtomId,
) -> bool {
    super::pdb::is_blacklisted_pair(editor.atoms(), begin, end)
}
...
fn rdkit_covalent_radius(atomic_number: u8) -> Result<f32, PdbPostprocessError> {
    ...
    RDKIT_COVALENT_RADII
        ...
        .copied()
        .ok_or(PdbPostprocessError::Unsupported(
...
fn other_atom_for_bond(bond: &Bond, atom: AtomId) -> AtomId {
    if bond.begin() == atom {
        bond.end()
    } else {
        bond.begin()
...
fn standard_pdb_double_bond_table_like_rdkit(rescode: u32, mut atm1: u32, mut atm2: u32) -> bool {
...
fn standard_pdb_double_bond_like_rdkit(
    ...
    if !super::pdb::same_pdb_residue(begin_info, end_info) {
        return false;
    }
...
fn basic_pdb_cleanup_like_rdkit(topology: &mut TopologyBlock) -> Result<(), PdbPostprocessError> {
    ...
    // RDKit✔️✔️:
    // RDKit✔️✔️:     // correct four-valent neutral N -> N+
    // RDKit✔️✔️:     // This was github #1029
    ...
    for atom_index in 0..topology.atoms.len() {
        ...
        if topology.atoms[atom_index].atomic_number() != 7
            || topology.atoms[atom_index].formal_charge() != 0
        {
            continue;
        }
        let explicit_valence = cosmolkit_core::calculate_explicit_valence_for_topology(
            topology, atom_id, false, false,
        )?;
...
/// Apply RDKit's chemistry-owned graph finalization to detached PDB blocks.
pub fn postprocess_pdb_detached(
    topology: &mut TopologyBlock,
    coordinates: &CoordinateBlock,
    params: PdbPostprocessParams,
) -> Result<(), PdbPostprocessError> {
    // RDKit source: PDBParser.cpp parsePdbBlock finalization
    // RDKit✔️✔️:   if (proximityBonding) {
    // RDKit✔️✔️:     ConnectTheDots(mol.get(), ctdIGNORE_H_H_CONTACTS);
    // RDKit✔️✔️:   }
    // RDKit✔️✔️:   if (proximityBonding || flavor & 8) {
    // RDKit✔️✔️:     StandardPDBResidueBondOrders(mol.get());
    // RDKit✔️✔️:   }
    // RDKit✔️✔️:   BasicPDBCleanup(*mol);
    topology.validate()?;
    coordinates.validate_for_atom_count(topology.atoms.len())?;
    {
        let mut editor = DetachedPdbEditor::new(topology, coordinates);
        if params.proximity_bonding {
            connect_the_dots_like_rdkit(&mut editor, CTD_IGNORE_H_H_CONTACTS)?;
        }
        if params.proximity_bonding || params.flavor & 8 != 0 {
            standard_pdb_residue_bond_orders_like_rdkit(&mut editor)?;
        }
        editor.finish();
    }
    basic_pdb_cleanup_like_rdkit(topology)?;
    topology.validate()?;
    Ok(())
}

/// Clear 3D-assigned chiral tags on nonstandard PDB residue atoms like RDKit.
pub fn apply_standard_pdb_residue_chirality_detached(
    topology: &mut TopologyBlock,
) -> Result<(), PdbPostprocessError> {
    topology.validate()?;
...
mod tests {
    ...
    fn chiral_atom(id: usize, atom_name: &str, residue_name: &str, is_hetero: bool) -> Atom {
        Atom::from_spec(
            AtomId::new(id),
            AtomSpec::new(Element::C)
                .with_chiral_tag(ChiralTag::TetrahedralCw)
                .with_prop("_CIPCode", "R")
                .expect("the internal CIP property key is non-empty")
    ...
    #[test]
    fn standard_residue_chirality_retains_only_source_allowlisted_polymer_atoms() {
        let mut topology = TopologyBlock {
            atoms: vec![
                chiral_atom(0, " CA ", "ALA", false),
                chiral_atom(1, " CB ", "ALA", false),
                chiral_atom(2, " X1 ", "LIG", true),
            ],
