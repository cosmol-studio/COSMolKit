use cosmolkit::{
    EmbedParameters, Molecule, mmff_has_all_molecule_params, mmff_optimize_molecule, uff_has_all_molecule_params,
    uff_optimize_molecule,
};

fn main() -> Result<(), Box<dyn std::error::Error>> {
    let molecule = Molecule::from_smiles("CC(=O)NC")?.with_hydrogens()?.sanitize()?;
    ...
    println!("single first atom={:?}", embedded.conformers_3d()[0].coordinates()[0]);
