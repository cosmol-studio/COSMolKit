...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    println!("full descriptor: {:?}", labeled.atoms()[1].cip_descriptor()?);
    ...
    println!("selected descriptor: {:?}", selected.atoms()[1].cip_descriptor()?);

    let mut double_bond = Molecule::from_smiles("F/C=C/F")?;
    double_bond.assign_cip_labels_()?;
    println!("double-bond descriptor: {:?}", double_bond.bonds()[1].cip_descriptor()?);
    Ok(())
}
