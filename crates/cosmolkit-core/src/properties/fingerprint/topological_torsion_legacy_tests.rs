...
use super::{
    FingerprintError, FingerprintFuncArguments, TopologicalTorsionArguments, getHashedTopologicalTorsionFingerprint,
    getHashedTopologicalTorsionFingerprintAsBitVect, getTopologicalTorsionFingerprint, getTopologicalTorsionGenerator,
...
fn legacy_unfolded_ids_counts_and_compatibility_size_match_pinned_rdkit() {
    ...
    let legacy = getTopologicalTorsionFingerprint(&molecule, 4, None, None, None, false).expect("legacy unfolded");
    ...
    let modern = getTopologicalTorsionGenerator(&TopologicalTorsionArguments::default(), None, true)
...
fn legacy_hashed_counts_match_exact_pinned_ids_and_modern_count_core() {
    ...
    let molecule = Molecule::from_smiles("CCCCO").expect("molecule");
    let legacy =
        getHashedTopologicalTorsionFingerprint(&molecule, 1000, 4, None, None, None, false).expect("legacy hashed");
    ...
    let modern = getTopologicalTorsionGeneratorWithParams(false, 4, None, true, 1000, vec![1, 2, 4, 8], true)
        .expect("modern generator")
...
fn legacy_bit_vector_uses_exact_four_bit_and_non_four_bit_thresholds() {
    ...
    let counts = getHashedTopologicalTorsionFingerprint(&molecule, 16, 4, None, None, Some(&invariants), false)
        .expect("block counts");
    ...
    let four =
        getHashedTopologicalTorsionFingerprintAsBitVect(&molecule, 64, 4, None, None, Some(&invariants), 4, false)
    ...
    let six =
        getHashedTopologicalTorsionFingerprintAsBitVect(&molecule, 96, 4, None, None, Some(&invariants), 6, false)
...
fn legacy_bit_vector_floors_non_divisible_block_sizes_and_keeps_tail_clear() {
    ...
    let invariants = vec![7; molecule.num_atoms()];
    let bits =
        getHashedTopologicalTorsionFingerprintAsBitVect(&molecule, 67, 4, None, None, Some(&invariants), 4, false)
...
fn legacy_selections_custom_invariants_and_chirality_use_the_shared_core() {
    ...
    let full =
        getHashedTopologicalTorsionFingerprint(&chain, 2048, 4, None, None, Some(&invariants), false).expect("full");
    let rooted = getHashedTopologicalTorsionFingerprint(&chain, 2048, 4, Some(&[0]), None, Some(&invariants), false)
        .expect("rooted");
    let ignored = getHashedTopologicalTorsionFingerprint(&chain, 2048, 4, None, Some(&[2]), Some(&invariants), false)
        .expect("ignored");
    ...
    let clockwise_achiral =
        getHashedTopologicalTorsionFingerprint(&clockwise, 2048, 4, None, None, None, false).unwrap();
    let anticlockwise_achiral =
        getHashedTopologicalTorsionFingerprint(&anticlockwise, 2048, 4, None, None, None, false).unwrap();
    assert_eq!(clockwise_achiral, anticlockwise_achiral);

    let clockwise_chiral = getHashedTopologicalTorsionFingerprint(&clockwise, 2048, 4, None, None, None, true).unwrap();
    let anticlockwise_chiral =
        getHashedTopologicalTorsionFingerprint(&anticlockwise, 2048, 4, None, None, None, true).unwrap();
...
fn legacy_invalid_parameters_return_structured_errors() {
    ...
    assert!(matches!(
        getHashedTopologicalTorsionFingerprintAsBitVect(&molecule, 64, 4, None, None, None, 0, false,),
        Err(FingerprintError::InvalidArguments { .. })
    ));
    assert!(matches!(
        getHashedTopologicalTorsionFingerprintAsBitVect(&molecule, 3, 4, None, None, None, 4, false,),
        Err(FingerprintError::InvalidArguments { .. })
    ));
    assert!(matches!(
