use super::{
    ...
    topological_fingerprint, topological_torsion_count_fingerprint, topological_torsion_fingerprint,
...
fn public_defaults_match_generator_defaults_and_remain_named_separately_from_rdkfingerprint() {
    ...
    let rdk = topological_fingerprint(&molecule, &TopologicalFingerprintParams::default()).expect("RDKFingerprint");
...
fn public_scalar_conveniences_expose_all_four_shared_vector_forms() {
    ...
    let sparse_count = topological_torsion_sparse_count_fingerprint(&molecule, &params).expect("sparse count");
    let sparse_bit = topological_torsion_sparse_fingerprint(&molecule, &params).expect("sparse bit");
...
fn public_output_request_returns_every_supported_provenance_allocation() {
    ...
    assert_eq!(output.atom_to_bits.as_ref().unwrap().len(), molecule.num_atoms());
    assert_eq!(output.atom_counts.as_ref().unwrap().len(), molecule.num_atoms());
...
fn public_legacy_compatibility_type_routes_all_three_legacy_forms() {
    ...
    let unfolded = topological_torsion_legacy_fingerprint(&molecule, &TopologicalTorsionLegacyParams::default())
        .expect("unfolded");
    assert!(matches!(unfolded, TopologicalTorsionLegacyResult::SparseCount(_)));
    ...
    assert_eq!(
        hashed.nonzero_elements().keys().copied().collect::<Vec<_>>(),
        vec![24, 288]
    );
