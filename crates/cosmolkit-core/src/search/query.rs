    ...
    fn matches(&self, atom: &Atom, mol: &Molecule, context: &QueryMatchContext) -> bool {
        let value = match self.data_function {
            ...
            AtomRangeDataFunction::ImplicitHydrogenCount => {
                query_atom_implicit_h_count(context.valence.as_ref(), atom)
                    .map(|value| value as i32)
            }
...
#[inline]
fn query_atom_has_heteroatom_nbrs(at: &Atom, adj: &AdjacencyList, mol: &Molecule) -> i32 {
...
#[inline]
fn query_atom_num_heteroatom_nbrs(at: &Atom, adj: &AdjacencyList, mol: &Molecule) -> i32 {
...
#[inline]
fn query_atom_has_aliphatic_heteroatom_nbrs(at: &Atom, adj: &AdjacencyList, mol: &Molecule) -> i32 {
...
#[inline]
fn query_atom_num_aliphatic_heteroatom_nbrs(at: &Atom, adj: &AdjacencyList, mol: &Molecule) -> i32 {
...
#[inline]
fn query_atom_bond_product(at: &Atom, adj: &AdjacencyList, mol: &Molecule) -> u32 {
...
pub fn atom_matches_query_with_context(
    ...
    match query {
        ...
        QueryNode::And(children) => and_query_match(children, false, |child| {
            atom_matches_query_with_context(atom, child, mol, ctx)
        }),
...
pub fn bond_matches_query_with_context(
    ...
    match query {
        ...
        QueryNode::And(children) => and_query_match(children, false, |child| {
            bond_matches_query_with_context(bond, child, mol, ctx)
        }),
...
fn query_atom_non_hydrogen_degree(at: &Atom, adj: &AdjacencyList, mol: &Molecule) -> u32 {
...
fn query_atom_heavy_atom_degree(at: &Atom, adj: &AdjacencyList, mol: &Molecule) -> u32 {
...
mod tests {
    ...
    fn smarts_query_bond_copy() {
        ...
        assert_eq!(&copied, source);
        copied.set_query(Some(QueryNode::predicate(BondQueryPredicate::Order(
            BondOrder::Single,
        ))));
        assert_ne!(copied.query(), source.query());
        assert_eq!(source.prop("label"), Some("source"));
    ...
    fn smarts_query_bond_set_type() {
        ...
        let bond_id = builder
            .add_bond(
                crate::BondSpec::new(begin, end, BondOrder::Single).with_query(QueryNode::and(
                    vec![
    ...
    fn smarts_query_bond_set_dir() {
        ...
        let bond_id = builder
            .add_bond(
                crate::BondSpec::new(begin, end, BondOrder::Double)
                    .with_query(original_query.clone()),
            )
    ...
    fn smarts_query_bond_query_match() {
        ...
        let query_single = builder
            .add_bond(
                crate::BondSpec::new(atoms[4], atoms[5], BondOrder::Single).with_query(
                    QueryNode::predicate(BondQueryPredicate::Order(BondOrder::Single)),
                ),
            )
        ...
        let query_double = builder
            .add_bond(
                crate::BondSpec::new(atoms[6], atoms[7], BondOrder::Double).with_query(
                    QueryNode::predicate(BondQueryPredicate::Order(BondOrder::Double)),
                ),
            )
    ...
    fn smarts_query_atom_dummy() {
        ...
        let negated_atom_null =
            builder.add_atom(crate::AtomSpec::new(crate::Element::DUMMY).with_query(
                QueryNode::not(QueryNode::predicate(AtomQueryPredicate::Any)),
            ));
        let composite_or = builder.add_atom(
            crate::AtomSpec::new(crate::Element::DUMMY).with_query(QueryNode::or(vec![
                QueryNode::predicate(AtomQueryPredicate::AtomicNumber(6)),
