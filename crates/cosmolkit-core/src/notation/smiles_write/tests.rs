...
fn shared_count_swaps_smiles_writer_preserves_invariant_failure_mapping() {
    let size_error =
        stereo::count_swaps_to_interconvert(&[BondId::new(0)], &[BondId::new(0), BondId::new(1)]).unwrap_err();
    assert!(matches!(
        size_error,
    ...
    let missing_error =
        stereo::count_swaps_to_interconvert(&[BondId::new(0), BondId::new(2)], &[BondId::new(0), BondId::new(1)])
            .unwrap_err();
    assert!(matches!(
        missing_error,
        SmilesWriteError::InvariantViolation {
...
fn mol_to_smiles_empty_molecule_returns_empty_string_like_rdkit_entrypoint() {
    ...
    assert_eq!(mol_to_smiles(&molecule, &SmilesWriteParams::default()).unwrap(), "");
}
...
fn all_primary_smiles_writer_modes_fail_closed_until_ported() {
    let molecule = ethane();
    assert_eq!(mol_to_smiles(&molecule, &SmilesWriteParams::default()).unwrap(), "CC");
    ...
    assert_eq!(random.len(), 2, "random SMILES should be 2 chars: {random:?}");
...
fn mol_to_smiles_updates_writer_property_cache_non_strict_like_rdkit() {
    ...
    let strict_error = crate::assign_valence(&molecule, crate::ValenceModel::RdkitLike).unwrap_err();
    assert!(
        strict_error
            .to_string()
...
fn mol_to_random_smiles_vect_returns_requested_count_and_is_seed_reproducible() {
    ...
    let first = mol_to_random_smiles_vect(&molecule, 8, 0x1234, false, false, false, false).unwrap();
    let second = mol_to_random_smiles_vect(&molecule, 8, 0x1234, false, false, false, false).unwrap();
    let different_seed = mol_to_random_smiles_vect(&molecule, 8, 0x5678, false, false, false, false).unwrap();
...
fn random_writer_seeded_start_matches_rdkit_for_diatomic_direction() {
    ...
    let generated = mol_to_random_smiles_vect(&molecule, 1, 1337, true, false, false, false).unwrap();
...
fn mol_to_smiles_with_mode_returns_empty_string_for_empty_molecule_like_rdkit() {
    ...
    let result =
        mol_to_smiles_with_mode(&molecule, &SmilesWriteParams::default(), SmilesOutputMode::PlainSmiles).unwrap();
...
fn prepare_plain_smiles_molecule_stashes_and_clears_atom_maps_like_rdkit() {
    ...
    assert_eq!(
        working.atoms().iter().map(|atom| atom.atom_map()).collect::<Vec<_>>(),
        vec![None, None, None]
    );
}
...
fn choose_fragment_start_atom_prefers_terminal_dummy_like_rdkit() {
    ...
    let plan = collect_fragment_write_plans(&molecule, &params)
        ...
        .next()
        .unwrap();
    let ranks = rank_fragment_atoms_for_smiles(&molecule, &plan, &params, SmilesOutputMode::PlainSmiles).unwrap();
...
fn collect_fragment_write_plans_tracks_fragment_root_and_bond_membership() {
    ...
    assert_eq!(plans[1].atoms, vec![AtomId::new(2), AtomId::new(3), AtomId::new(4)]);
...
fn canonical_writer_uses_traversal_order_for_tetrahedral_chirality() {
    ...
    assert_eq!(mol_to_smiles(&molecule, &params).unwrap(), "[C@@H](C)(F)CCCl");
...
fn rooted_canonical_writer_matches_rdkit_branch_order_for_multichiral_case() {
    ...
    assert_eq!(mol_to_smiles(&molecule, &params).unwrap(), "O[C@](C)(Cl)[C@@](C)(O)Cl");
}
...
fn shared_canon_helpers_writer_treats_single_h_query_as_fourth_valence() {
    ...
    assert!(!chiral_atom_needs_tag_inversion_for_writer(&molecule, center, false, 1));
}
...
fn canonical_dfs_traversal_inserts_implicit_nontetrahedral_neighbors_like_rdkit() {
    ...
    insert_implicit_nontetrahedral_neighbors_for_writer(&mut first_atom_bonds, ChiralTag::SquarePlanar, true);
    assert_eq!(
        first_atom_bonds,
    ...
    insert_implicit_nontetrahedral_neighbors_for_writer(&mut later_atom_bonds, ChiralTag::SquarePlanar, false);
    assert_eq!(
        later_atom_bonds,
...
fn canonical_dfs_traversal_marks_broken_chirality_for_partial_plan() {
    ...
    let plan = FragmentWritePlan {
        atoms: vec![AtomId::new(0), AtomId::new(1), AtomId::new(2), AtomId::new(3)],
...
fn writer_uses_parser_persisted_ring_stereo_props_without_local_reconstruction() {
    ...
    assert_eq!(mol_to_smiles(&molecule, &params).unwrap(), "C1[C@H](F)CC[C@H](Cl)C1");
}
...
fn writer_rejects_malformed_ring_stereo_neighbors_prop() {
    let mut molecule = Molecule::from_smiles_with_sanitize("C1[C@H](F)CC[C@H](Cl)C1", true).unwrap();
...
fn mol_fragment_to_smiles_rejects_rooted_atom_for_multifragment_molecule_without_bond_scope() {
    ...
    assert_eq!(error, SmilesWriteError::RootedAtomRequiresSingleFragment { atom: 0 });
}
...
fn isomeric_writer_outputs_non_tetrahedral_chiral_classes_like_rdkit() {
    ...
    let trigonal_bipyramidal = Molecule::from_smiles_with_sanitize("[P@TB20](F)(Cl)(Br)(I)C", false).unwrap();
    let octahedral = Molecule::from_smiles_with_sanitize("[Co@OH30](F)(Cl)(Br)(I)(N)C", false).unwrap();
...
fn isomeric_writer_outputs_default_non_tetrahedral_chiral_classes_like_rdkit() {
    ...
    let trigonal_bipyramidal = Molecule::from_smiles_with_sanitize("[P@TB](F)(Cl)(Br)(I)C", false).unwrap();
    let octahedral = Molecule::from_smiles_with_sanitize("[Co@OH](F)(Cl)(Br)(I)(N)C", false).unwrap();
...
fn canonical_fragment_assembly_reorders_output_scope_with_sorted_smiles() {
    ...
    let smiles =
        mol_fragment_to_smiles_with_context(&molecule, &params, &[0, 1], None, None, None, &mut context).unwrap();

    assert_eq!(smiles, "C.O");
    assert_eq!(context.atom_output_order, vec![AtomId::new(1), AtomId::new(0)]);
}
...
fn noncanonical_nonisomeric_plain_smiles_writes_bracket_atoms_from_explicit_state() {
    ...
    assert_eq!(mol_to_smiles(&molecule, &params).unwrap(), "[NH4+].[O-].[SiH2].[*:7]");
}
...
fn get_bond_smiles_ignores_direction_on_double_bond_like_rdkit() {
    ...
    let c2 = builder.add_atom(crate::AtomSpec::new(crate::Element::C));
    builder
        .add_bond(crate::BondSpec::new(c1, c2, BondOrder::Double).with_direction(BondDirection::BeginWedge))
        .unwrap();
    let molecule = builder.build().unwrap();
    let params = SmilesWriteParams {
        do_isomeric_smiles: false,
    ...
    assert_eq!(get_molecule_bond_smiles(&molecule, 0, Some(0), &params).unwrap(), "=");
}
...
fn cx_smiles_restore_bond_dirs_clear_uses_working_copy_only() {
    ...
    let c2 = builder.add_atom(crate::AtomSpec::new(crate::Element::C));
    builder
        .add_bond(crate::BondSpec::new(c1, c2, BondOrder::Single).with_direction(BondDirection::Unknown))
        .unwrap();
    let molecule = builder.build().unwrap();
    let params = SmilesWriteParams {
        do_isomeric_smiles: false,
    ...
    assert_eq!(
        mol_to_cx_smiles(&molecule, &params, CxSmilesFields::ALL, RestoreBondDirOption::Clear,).unwrap(),
        "CC"
    );
...
fn noncanonical_nonisomeric_plain_smiles_writes_aromatic() {
    ...
    assert!(smi.contains('c'), "expected lowercase c for aromatic atoms, got: {smi}");
}
...
fn noncanonical_nonisomeric_plain_smiles_writes_zero_order_and_radical_bonds_with_rdkit_compatibility() {
    ...
    assert!(output.contains('~'), "zero-order bond should map to ~, got: {output:?}");
    ...
    assert!(output.contains('['), "radical atom needs bracket: {output:?}");
}
...
fn writer_kekule_all_hydrogens_explicit_preserves_pyrrolic_hydrogen_like_rdkit() {
    ...
    assert_eq!(mol_to_smiles(&mol, &params).unwrap(), "[NH]1[CH]=[CH][CH]=[CH]1");
}
...
fn writer_restores_atom_maps_before_kekulize_like_rdkit_row_142() {
    ...
    let plan = collect_fragment_write_plans(&molecule, &params)
        ...
        .next()
        .unwrap();
    let _ranks = rank_fragment_atoms_for_smiles(&molecule, &plan, &params, SmilesOutputMode::PlainSmiles).unwrap();
...
fn writer_canonicalizes_fused_ring_double_bond_directions_like_rdkit() {
    ...
    let fixtures = [
        ...
        ("C1/C=C\\C2CCCCC2C1", "C1=CC2CCCCC2CC1", "C1=C\\C2CCCCC2CC/1"),
        ("C1C/C=C/CC2CCCCC12", "C1=CCC2CCCCC2CC1", "C1=C/CC2CCCCC2CC/1"),
        ("C1/C=C/C2=C/CCCC2C1", "C1=CC2=CCCCC2CC1", "C1=C2/C=C/CCC2CCC1"),
        ("C1/C=C\\C2=C/CCCC2C1", "C1=CC2=CCCCC2CC1", "C1=C2/C=C\\CCC2CCC1"),
        (
...
fn writer_fragment_temp_graph_resets_ring_info_like_rdkit() {
    for (input, clears_computed) in [("C1CCCCC1", false), ("C1CCCCC1.[Na+]", true)] {
        ...
        molecule.properties_mut().set_prop("ordinary-molecule", "keep");
        ...
        assert_eq!(
            ...
            if clears_computed { None } else { Some("remove") },
            "input={input}"
        );
        assert_eq!(molecule.prop("ordinary-molecule"), Some("keep"), "input={input}");
        assert_eq!(
            molecule.atoms()[0].prop("computed-atom"),
            if clears_computed { None } else { Some("remove") },
            "input={input}"
        );
        assert_eq!(molecule.atoms()[0].prop("ordinary-atom"), Some("keep"), "input={input}");
        assert_eq!(
            molecule.bonds()[0].prop("computed-bond"),
            if clears_computed { None } else { Some("remove") },
            "input={input}"
        );
        assert_eq!(molecule.bonds()[0].prop("ordinary-bond"), Some("keep"), "input={input}");
    }
}
...
fn writer_fragment_reassignment_sets_double_bond_stereo_from_directions_like_rdkit() {
    ...
    let end_directed = incident_bonds(&molecule, end)
        ...
        .find(|bond| *bond != double_bond)
        .unwrap();
    molecule.topology_block_mut().bonds[begin_directed.index()].set_direction(BondDirection::EndUpRight);
    molecule.topology_block_mut().bonds[end_directed.index()].set_direction(BondDirection::EndDownRight);
...
fn debug_probe_rust_writer_fused_ring_chain() {
    ...
    eprintln!(
        ...
        mol_to_smiles(&Molecule::from_smiles_with_sanitize(input, false).unwrap(), &params).unwrap()
    );
}
...
fn debug_probe_rust_writer_row_91_direction_canonicalization() {
    ...
    let mut molecule = Molecule::from_smiles_with_sanitize(input, false).unwrap();
    let _ = prepare_plain_smiles_molecule(&mut molecule, &params).unwrap();
    let plan = collect_fragment_write_plans(&molecule, &params).unwrap().remove(0);
    let ranks = rank_fragment_atoms_for_smiles(&molecule, &plan, &params, SmilesOutputMode::PlainSmiles).unwrap();
    let start_atom = choose_fragment_start_atom(&plan, &ranks, &params).unwrap();
    let traversal = canonicalize_fragment_stack(
        &molecule,
...
fn canonicalize_double_bond_clear_bond_dirs_does_not_require_neighboring_stereo_double_bond() {
    ...
    let ref_bond = builder
        .add_bond(crate::BondSpec::new(center, fluorine, BondOrder::Single).with_direction(BondDirection::EndUpRight))
        .unwrap();
    builder
        .add_bond(crate::BondSpec::new(center, chlorine, BondOrder::Single).with_direction(BondDirection::EndDownRight))
        .unwrap();
    builder
...
fn remove_redundant_bond_dir_specs_requires_neighboring_stereo_double_bond_like_rdkit() {
    ...
    let ref_bond = builder
        .add_bond(crate::BondSpec::new(a0, a1, BondOrder::Single).with_direction(BondDirection::EndUpRight))
        .unwrap();
    ...
    remove_redundant_bond_dir_specs_for_writer(&mut molecule, &stack, &mut bond_dir_counts, &mut atom_dir_counts);
...
fn writer_plain_nonisomeric_row_91_matches_rdkit_after_direction_cleanup() {
    ...
    assert_eq!(mol_to_smiles(&mol, &params).unwrap(), "O=C1-C(=C(-C)-C)-C-C-C(-C)-C-1");
}
        ...
        match *item {
            ...
            MolStackElem::Bond(bond, _) => {
                ...
                if matches!(
                    ...
                    canonicalized.topology_block_mut().bonds[bond.index()].set_direction(BondDirection::None);
                }
            }
...
fn writer_swap_counter_reports_invariant_for_length_mismatch() {
    let error = count_swaps_to_interconvert(&[BondId::new(0)], &[BondId::new(0), BondId::new(1)]).unwrap_err();
...
fn writer_swap_counter_reports_invariant_for_missing_expected_bond() {
    let error =
        count_swaps_to_interconvert(&[BondId::new(0), BondId::new(1)], &[BondId::new(0), BondId::new(2)]).unwrap_err();
...
fn atom_bond_and_fragment_writer_entries_fail_closed_until_ported() {
    ...
    let fragment = mol_fragment_to_smiles(&molecule, &SmilesWriteParams::default(), &[0, 1], None, None, None).unwrap();
...
fn atom_bond_and_fragment_writer_entries_validate_indices_before_unsupported() {
    ...
    assert_eq!(
        mol_fragment_to_smiles(&molecule, &SmilesWriteParams::default(), &[2], None, None, None).unwrap_err(),
        SmilesWriteError::AtomOutOfRange { atom: 2 }
    );
}
...
fn mol_fragment_to_cx_smiles_filters_cx_blocks_to_fragment_output_scope() {
    let mut builder = crate::MoleculeBuilder::new();
    let c0 = builder.add_atom(crate::AtomSpec::new(crate::Element::C).with_prop("_supplementalSmilesLabel", "keep"));
    let c1 = builder.add_atom(crate::AtomSpec::new(crate::Element::C));
    let o2 = builder.add_atom(crate::AtomSpec::new(crate::Element::O).with_prop("_supplementalSmilesLabel", "drop"));
    builder
...
fn cx_ethanol() -> Molecule {
    ...
    let c1 = builder.add_atom(crate::AtomSpec::new(crate::Element::C));
    let c2 = builder.add_atom(crate::AtomSpec::new(crate::Element::C));
    let o = builder.add_atom(crate::AtomSpec::new(crate::Element::O).with_prop("atomLabel", "Hydroxy"));
    builder
        .add_bond(crate::BondSpec::new(c1, c2, crate::BondOrder::Single))
        .unwrap();
    builder
...
fn cx_individual_coords_writes_atom_order_coordinates() {
    ...
    builder
        .add_bond(crate::BondSpec::new(c1, c2, crate::BondOrder::Single))
        .unwrap();
    builder.set_2d_coordinates(vec![[0.0, 0.0], [1.5, 0.0]]).unwrap();
    let molecule = builder.build().unwrap();
...
fn cx_bond_cfg_writes_wedge_and_dash_when_coords_are_included() {
    ...
    builder
        .add_bond(
            crate::BondSpec::new(a0, a1, crate::BondOrder::Single).with_direction(crate::BondDirection::BeginWedge),
        )
        .unwrap();
    builder
        .add_bond(
            crate::BondSpec::new(a1, a2, crate::BondOrder::Single).with_direction(crate::BondDirection::BeginDash),
        )
        .unwrap();
    builder
    ...
    let extension = get_cx_extensions(&molecule, CxSmilesFields::COORDS | CxSmilesFields::BOND_CFG).unwrap();
    assert!(
...
fn cx_bond_cfg_uses_molfile_bond_cfg_fallback_for_wedge_and_dash() {
    ...
    let a1 = builder.add_atom(crate::AtomSpec::new(crate::Element::C));
    builder
        .add_bond(crate::BondSpec::new(a0, a1, crate::BondOrder::Single).with_prop("_MolFileBondCfg", "3"))
        .unwrap();
    builder.set_2d_coordinates(vec![[0.0, 0.0], [1.5, 0.0]]).unwrap();
    let molecule = builder.build().unwrap();
...
fn cx_bond_cfg_emits_unknown_without_coords_from_molfile_cfg() {
    ...
    let a1 = builder.add_atom(crate::AtomSpec::new(crate::Element::C));
    builder
        .add_bond(crate::BondSpec::new(a0, a1, crate::BondOrder::Single).with_prop("_MolFileBondCfg", "2"))
        .unwrap();
    let molecule = builder.build().unwrap();
...
fn cx_bond_atropisomer_writes_wedge_for_atrop_neighbor() {
    ...
    builder
        ...
        .unwrap();
    builder
        .add_bond(crate::BondSpec::new(a0, a2, crate::BondOrder::Single).with_stereo(crate::BondStereo::AtropCw))
        .unwrap();
    let molecule = builder.build().unwrap();
...
fn cx_bond_atropisomer_does_not_use_molfile_cfg_fallback() {
    ...
    let a2 = builder.add_atom(crate::AtomSpec::new(crate::Element::C));
    builder
        .add_bond(crate::BondSpec::new(a0, a1, crate::BondOrder::Single).with_prop("_MolFileBondCfg", "1"))
        .unwrap();
    builder
        .add_bond(crate::BondSpec::new(a0, a2, crate::BondOrder::Single).with_stereo(crate::BondStereo::AtropCw))
        .unwrap();
...
fn cx_ring_double_bond_molecule(stereo: BondStereo) -> Molecule {
    ...
    builder
        ...
        .unwrap();
    builder
        .add_bond(crate::BondSpec::new(atoms[1], atoms[2], crate::BondOrder::Single))
        .unwrap();
    builder
        .add_bond(crate::BondSpec::new(atoms[2], atoms[3], crate::BondOrder::Single))
        .unwrap();
    builder
        .add_bond(crate::BondSpec::new(atoms[3], atoms[4], crate::BondOrder::Single))
        .unwrap();
    builder
        .add_bond(crate::BondSpec::new(atoms[4], atoms[5], crate::BondOrder::Single))
        .unwrap();
    builder
        .add_bond(crate::BondSpec::new(atoms[5], atoms[6], crate::BondOrder::Single))
        .unwrap();
    builder
        .add_bond(crate::BondSpec::new(atoms[6], atoms[7], crate::BondOrder::Single))
        .unwrap();
    builder
        .add_bond(crate::BondSpec::new(atoms[7], atoms[0], crate::BondOrder::Single))
        .unwrap();
    let mut molecule = builder.build().unwrap();
...
fn cx_linknodes_block_writes_compact_form_for_degree_two_center() {
    ...
    builder
        ...
        .unwrap();
    let molecule = builder.build().unwrap().with_prop("_MolFileLinkNodes", "1 3 2 2 1 2 3");
...
fn cx_linknodes_block_writes_outer_atoms_for_degree_three_center() {
    ...
    builder
        ...
        .unwrap();
    let molecule = builder.build().unwrap().with_prop("_MolFileLinkNodes", "1 4 2 1 2 1 3");
...
fn cx_polymer_block_writes_copolymer_subtype_and_crossings() {
    ...
    builder
        .add_substance_group(
            crate::SubstanceGroup::new(crate::SubstanceGroupId::new(0), crate::SubstanceGroupKind::Copolymer)
                .with_atoms(vec![a0, a1])
                .with_subtype("ALT")
                .with_label("COP")
...
fn cx_sgroup_hierarchy_writes_block_for_sgroup_parent_prop() {
    ...
    builder
        .add_substance_group(
            crate::SubstanceGroup::new(crate::SubstanceGroupId::new(0), crate::SubstanceGroupKind::Data)
                .with_atoms(vec![a0])
                .with_prop("index", "0"),
        )
        .unwrap();
...
fn cx_full_molecule_with_all_fields_returns_smiles_with_cx_extension() {
    ...
    assert!(result.contains("Hydroxy"), "CX label should appear: {result:?}");
}
...
fn cx_get_cx_extensions_returns_union_of_requested_fields() {
    ...
    assert!(!result.contains('('), "no coords (molecule has no 2D coords)");
}
...
fn cx_individual_atom_props_escapes_property_name_and_value_dots() {
    let mut builder = crate::MoleculeBuilder::new();
    let c = builder.add_atom(crate::AtomSpec::new(crate::Element::C).with_prop("foo.bar", "baz.qux"));
    let c2 = builder.add_atom(crate::AtomSpec::new(crate::Element::C));
    builder
...
fn cx_molfile_values_when_present() {
    let mut builder = crate::MoleculeBuilder::new();
    let c = builder.add_atom(crate::AtomSpec::new(crate::Element::C).with_prop("molFileValue", "test_value"));
    let c2 = builder.add_atom(crate::AtomSpec::new(crate::Element::C));
    builder
        .add_bond(crate::BondSpec::new(c, c2, crate::BondOrder::Single))
        .unwrap();
...
fn cx_stereo_group_writes_appropriate_code() {
    ...
    builder
        ...
        .unwrap();
    builder
        .add_stereo_group(crate::StereoGroup::new(crate::StereoGroupKind::Or, vec![a1], vec![]))
        .unwrap();
    builder
        .add_stereo_group(crate::StereoGroup::new(crate::StereoGroupKind::And, vec![a0], vec![]))
        .unwrap();
    let molecule = builder.build().unwrap();
    let scope = CxWriteScope::full_molecule(&molecule);
...
fn cleanup_stereo_groups_for_cx_smiles_moves_atrop_atoms_to_bond_membership() {
    ...
    builder
        ...
        .unwrap();
    builder
        .add_bond(crate::BondSpec::new(a1, a2, crate::BondOrder::Single).with_stereo(crate::BondStereo::AtropCw))
        .unwrap();
    builder
...
fn writer_preserves_non_ring_aromatic_bridge_as_explicit_single_like_rdkit_biphenyl() {
    ...
    assert_eq!(molecule.to_smiles_with_params(&params).unwrap(), "c1ccc(-c2ccccc2)cc1");
}

#[test]
fn writer_emits_single_bridge_between_aromatic_rings_like_rdkit_row_90() {
    let molecule = Molecule::from_smiles("CCOC(=O)c1c(N/C=C\\2/C(=NC(=NC2=O)S)O)scc1c1ccc(cc1)Br").unwrap();
    let params = SmilesWriteParams {
        do_isomeric_smiles: false,
        canonical: false,
        clean_stereo: false,
...
fn writer_preserves_bridgehead_tetrahedral_stereo_like_rdkit_row_110() {
    let molecule =
        Molecule::from_smiles("[H]Cl.NC[C@@H]1O[C@@H](CC2=C(O)C(O)=CC=C12)C34C[C@H](C5)C[C@H](C[C@H]5C4)C3").unwrap();
    let params = SmilesWriteParams {
        do_isomeric_smiles: true,
        canonical: false,
        clean_stereo: false,
...
fn canonical_fragment_scope_matches_every_retained_stress_profile() {
    let fixture = include_str!("../../../../../testdata/smiles/fixtures/rdkit_fragment_scope_stress_regressions.jsonl");
    let mut records = 0usize;
    let mut cases = std::collections::BTreeSet::new();

    for (line_idx, line) in fixture.lines().enumerate() {
        let record: FragmentScopeStressRecord = serde_json::from_str(line)
            .unwrap_or_else(|error| panic!("invalid fragment-scope stress fixture line {}: {error}", line_idx + 1));
    ...
    assert_eq!(cases.len(), 9, "fixture must retain every affected molecule");
}
...
fn debug_row_121_noncanonical_traversal() {
    ...
    let plan = collect_fragment_write_plans(&molecule, &working_params)
        ...
        .unwrap();
    let ranks =
        rank_fragment_atoms_for_smiles(&molecule, &plan, &working_params, SmilesOutputMode::PlainSmiles).unwrap();
    ...
    for atom_idx in [17usize, 35, 33, 26, 32, 36, 39, 40] {
        ...
        for bond in &atom_ring_closures[atom_idx] {
            ...
            let other = if bond.begin() == atom { bond.end() } else { bond.begin() };
...
fn debug_row_142_canonical_kekule_root_last_traversal() {
    ...
    eprintln!();
    eprintln!("output={}", molecule.to_smiles_with_params(&params).unwrap());
}
