...
fn parse_uff_param_data() -> HashMap<String, UffAtomicParams> {
    let cpp = include_str!("../../../third_party/rdkit/Code/ForceField/UFF/Params.cpp");
    let start = cpp
        .find("const std::string defaultParamData =")
        .expect("RDKit UFF params string must exist");
    let rest = &cpp[start..];
    let end = rest.find("// clang-format on").unwrap_or(rest.len());
    ...
    for line in rest[..end].lines() {
        ...
        let frag = &trimmed[1..close];
        encoded.push_str(frag);
    }
    ...
    for line in decoded.lines() {
        ...
        out.insert(
            cols[0].to_string(),
            UffAtomicParams {
                r1: cols[1].parse().expect("valid UFF r1"),
                gmp_xi: cols[9].parse().expect("valid UFF Xi"),
            },
        );
    }
    out
...
fn uff_params() -> &'static HashMap<String, UffAtomicParams> {
    static UFF_PARAMS: OnceLock<HashMap<String, UffAtomicParams>> = OnceLock::new();
    UFF_PARAMS.get_or_init(parse_uff_param_data)
}

fn parse_periodic_table_rvdw() -> HashMap<u8, f64> {
    let cpp = include_str!("../../../third_party/rdkit/Code/GraphMol/atomic_data.cpp");
    let start = cpp
        .find("const std::string periodicTableAtomData =")
        .expect("RDKit periodic table data must exist");
    let rest = &cpp[start..];
    let end = rest.find(";").unwrap_or(rest.len());
    let mut joined = String::new();
    let mut cursor = &rest[..end];
    while let Some(pos) = cursor.find("R\"DAT(") {
        let after = &cursor[pos + 6..];
        let Some(close) = after.find(")DAT\"") else {
            break;
        };
        joined.push_str(&after[..close]);
        cursor = &after[close + 5..];
    }
    let mut out = HashMap::new();
    for line in joined.lines() {
        ...
        let atomic_num: u8 = cols[0].parse().expect("valid atomic number");
        let rvdw: f64 = cols[5].parse().expect("valid rvdw");
        out.insert(atomic_num, rvdw);
    }
    out
}

fn periodic_table_rvdw() -> &'static HashMap<u8, f64> {
    static RVDW: OnceLock<HashMap<u8, f64>> = OnceLock::new();
    RVDW.get_or_init(parse_periodic_table_rvdw)
}
