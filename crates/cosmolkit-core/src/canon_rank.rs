...
impl CanonRankFlags {
    const fn from_fragment_options(options: CanonicalRankOptions) -> Self {
        Self {
            ...
            // RDKit rankFragmentAtoms sets df_useChiralityRings from
            // includeChirality, while includeRingStereo only controls the
            // special ring-stereo refinement pass inside rankWithFunctor.
            use_chirality_rings: options.include_chirality,
...
pub(crate) fn rank_fragment_atoms(
    ...
    // BEGIN RDKIT CPP FUNCTION Canon::rankFragmentAtoms
    // RDKit❗✔️: void rankFragmentAtoms(const ROMol &mol, std::vector<unsigned int> &res,
    ...
    // RDKit✔️✔️:     return;
    // RDKit✔️✔️:   }
    // RDKit❗✔️:   bool clearRings = false;
    // RDKit❗✔️:   if (!mol.getRingInfo()->isFindFastOrBetter()) {
    // RDKit❗✔️:     MolOps::fastFindRings(mol);
    // RDKit❗✔️:     clearRings = true;
    // RDKit❗✔️:   }
    // RDKit❗✔️:   res.resize(mol.getNumAtoms());
    // RDKit❗✔️:   std::vector<Canon::canon_atom> atoms(mol.getNumAtoms());
    // RDKit❗✔️:   detail::initFragmentCanonAtoms(mol, atoms, includeChirality, atomSymbols,
    // RDKit❗✔️:                                  bondSymbols, atomsInPlay, bondsInPlay, true);
    // RDKit❗✔️:   AtomCompareFunctor ftor(&atoms.front(), mol, &atomsInPlay, &bondsInPlay);
    // RDKit❗✔️:   ftor.df_useIsotopes = includeIsotopes;
    // RDKit❗✔️:   ftor.df_useChirality = includeChirality;
    // RDKit❗✔️:   ftor.df_useAtomMaps = includeAtomMaps;
    // RDKit❗✔️:   ftor.df_useChiralityRings = includeChirality;
    // RDKit❗✔️:   ftor.df_useChiralPresence = includeChiralPresence;
    // RDKit❗✔️:   std::vector<int> order(mol.getNumAtoms());
    // RDKit❗✔️:   detail::rankWithFunctor(ftor, breakTies, order, true, includeChirality,
    // RDKit❗✔️:                           includeRingStereo, &atomsInPlay, &bondsInPlay);
    // RDKit❗✔️:   for (unsigned int i = 0; i < mol.getNumAtoms(); ++i) {
    // RDKit❗✔️:     res[order[i]] = atoms[order[i]].index;
    // RDKit❗✔️:   }
    // RDKit❗✔️:   if (clearRings) {
    // RDKit❗✔️:     mol.getRingInfo()->reset();
    // RDKit❗✔️:   }
    // RDKit❗✔️: }
    // END RDKIT CPP FUNCTION Canon::rankFragmentAtoms
...
fn init_fragment_canon_atoms_for_kekulize<'a>(
    ...
    // RDKit❗✔️: void initFragmentCanonAtoms(const ROMol &mol,
    // RDKit❗✔️:                             std::vector<Canon::canon_atom> &atoms,
    // RDKit✔️✔️:                             bool includeChirality,
    // RDKit✔️✔️:                             const std::vector<std::string> *atomSymbols,
    // RDKit✔️✔️:                             const std::vector<std::string> *bondSymbols,
    // RDKit❗✔️:                             const boost::dynamic_bitset<> &atomsInPlay,
    // RDKit❗✔️:                             const boost::dynamic_bitset<> &bondsInPlay,
    // RDKit❗✔️:                             bool needsInit) {
    ...
    // RDKit✔️✔️:       }
    // RDKit❗✔️:       if (needsInit) {
    // RDKit❗✔️:         atomsi.nbrIds = std::make_unique<int[]>(atom->getDegree());
    // RDKit✔️✔️:         advancedInitCanonAtom(mol, atomsi, i);
    // RDKit❗✔️:         atomsi.bonds.reserve(4);
    // RDKit❗✔️:       }
    // RDKit❗✔️:     }
    // RDKit✔️✔️:   }
    ...
    advanced_init_canon_atoms_stereo_state_for_kekulize(molecule, &mut atoms, atoms_to_use)?;
    // RDKit✔️✔️: }
...
fn advanced_init_canon_atoms_stereo_state_for_kekulize(
    ...
    // RDKit❗✔️:   if (includeChirality && includeStereoGroups) {
    ...
    for (group_idx, group) in molecule.stereo_groups().iter().enumerate() {
        ...
        for atom in group.atoms() {
            ...
            if atom_idx < atoms.len() && atoms_to_use[atom_idx] {
...
fn make_canon_bond_holder<'a>(
    ...
    // RDKit❗✔️: bondholder makeBondHolder(const Bond *bond, unsigned int otherIdx,
    // RDKit❗✔️:                           bool includeChirality,
    // RDKit❗✔️:                           const std::vector<Canon::canon_atom> &atoms) {
...
fn rank_with_atom_compare_functor_for_kekulize(
    ...
    // RDKit❗✔️: void rankWithFunctor(T &ftor, bool breakTies, std::vector<int> &order,
    // RDKit❗✔️:                      bool useSpecial, bool useChirality, bool includeRingStereo,
    // RDKit❗✔️:                      const boost::dynamic_bitset<> *atomsInPlay,
    // RDKit❗✔️:                      const boost::dynamic_bitset<> *bondsInPlay) {
...
fn refine_partitions_for_kekulize(
    ...
    // RDKit❗✔️: template <typename CompareFunc>
    // RDKit❗✔️: void RefinePartitions(const ROMol &mol, canon_atom *atoms, CompareFunc compar,
    // RDKit❗✔️:                       int mode, std::vector<int> &order,
    // RDKit❗✔️:                       std::vector<int> &count, int &activeset,
    // RDKit❗✔️:                       std::vector<int> &next, std::vector<int> &changed,
    // RDKit❗✔️:                       std::vector<char> &touchedPartitions) {
    ...
    while *active_set != -1 {
        ...
        *active_set = next[partition];
        ...
        // RDKit❗✔️:     auto start = std::span<int>(&order[offset], len);
...
fn break_ties_for_kekulize(
    ...
    // RDKit❗✔️: template <typename CompareFunc>
    // RDKit❗✔️: void BreakTies(const ROMol &mol, canon_atom *atoms, CompareFunc compar,
    // RDKit❗✔️:                int mode, std::vector<int> &order, std::vector<int> &count,
    // RDKit❗✔️:                int &activeset, std::vector<int> &next,
    // RDKit❗✔️:                std::vector<int> &changed,
    // RDKit❗✔️:                std::vector<char> &touchedPartitions) {
    // RDKit✔️✔️:   unsigned int nAtoms = mol.getNumAtoms();
    let n_atoms = molecule.num_atoms();
    // RDKit✔️✔️:   for (unsigned int i = 0; i < nAtoms; i++) {
    ...
    while i < n_atoms {
        ...
        // RDKit✔️✔️:       i -= 1;
        // RDKit✔️✔️:     }
        if atoms[partition].index != old_part && i > 0 {
            i -= 1;
        } else {
            i += 1;
        }
...
fn hanoi_sort_order_for_kekulize(
    ...
    // RDKit✔️❌: template <typename CompareFunc>
    // RDKit✔️❌: void hanoisort(std::span<int> &base, std::vector<int> &count,
    // RDKit✔️❌:                std::vector<int> &changed, CompareFunc compar) {
    // RDKit✔️❌:   std::vector<int> tempVec(base.size());
    let mut temp = vec![0usize; len];
    // RDKit✔️❌:   if (detail::hanoi(base.data(), base.size(), tempVec.data(), count.data(),
    // RDKit✔️❌:                     changed.data(), compar)) {
    if hanoi_order_for_kekulize(
        ...
        // RDKit✔️❌:     std::copy(tempVec.begin(), tempVec.end(), base.begin());
...
fn hanoi_order_for_kekulize(
    ...
    // RDKit✔️❌: template <typename CompareFunc>
    // RDKit✔️❌: bool hanoi(int *base, int nel, int *temp, int *count, int *changed,
    // RDKit✔️❌:            CompareFunc compar) {
    // RDKit✔️❌:   assert(base);
    // RDKit✔️❌:   assert(temp);
    // RDKit✔️❌:   assert(count);
    // RDKit✔️❌:   assert(changed);
    ...
    // RDKit✔️❌:   if (nel == 1) {
    // RDKit✔️❌:     count[base[0]] = 1;
    // RDKit✔️❌:     return false;
    // RDKit✔️❌:   } else if (nel == 2) {
    if nel == 1 {
        count[base[0]] = 1;
        return false;
    } else if nel == 2 {
        ...
        let n2 = base[1];
        // RDKit✔️❌:     int stat =
        // RDKit✔️❌:         (/*!changed || */ changed[n1] || changed[n2]) ? compar(n1, n2) : 0;
        ...
        // RDKit✔️❌:       count[n1] = 1;
        // RDKit✔️❌:       count[n2] = 1;
        // RDKit✔️❌:       base[0] = n2; /* temp[0] = n2; */
        // RDKit✔️❌:       base[1] = n1; /* temp[1] = n1; */
        // RDKit✔️❌:       return false; /* return True;  */
        // RDKit✔️❌:     }
        match stat {
            Ordering::Equal => {
                count[n1] = 2;
    ...
    // RDKit✔️❌:   if (hanoi(b1, n1, t1, count, changed, compar)) {
    ...
    // RDKit✔️❌:     if (hanoi(b2, n2, t2, count, changed, compar)) {
    ...
    let left_source = if left_in_temp {
        // RDKit✔️❌:     s1 = t1;
        temp_left.to_vec()
    } else {
        // RDKit✔️❌:     s1 = b1;
    ...
    let right_source = if right_in_temp {
        // RDKit✔️❌:       s2 = t2;
        temp_right.to_vec()
    } else {
        // RDKit✔️❌:       s2 = b2;
    ...
    let output = if result { temp } else { base };
    ...
    // RDKit✔️❌:   while (true) {
    loop {
        // RDKit✔️❌:     assert(*s1 != *s2);
        debug_assert_ne!(left_source[left_pos], right_source[right_pos]);
        // RDKit✔️❌:     int stat =
        // RDKit✔️❌:         (/*!changed || */ changed[*s1] || changed[*s2]) ? compar(*s1, *s2) : 0;
        let left_atom = left_source[left_pos];
        let right_atom = right_source[right_pos];
        ...
        // RDKit✔️❌:     int len1 = count[*s1];
        // RDKit✔️❌:     int len2 = count[*s2];
        // RDKit✔️❌:     assert(len1 > 0);
        // RDKit✔️❌:     assert(len2 > 0);
        ...
        if stat == Ordering::Equal {
            // RDKit✔️❌:       count[*s1] = len1 + len2;
            // RDKit✔️❌:       count[*s2] = 0;
            count[left_atom] = class_left + class_right;
            count[right_atom] = 0;
            // RDKit✔️❌:       memmove(ptr, s1, len1 * sizeof(int));
            output[out_pos..out_pos + class_left]
                .copy_from_slice(&left_source[left_pos..left_pos + class_left]);
            // RDKit✔️❌:       ptr += len1;
            // RDKit✔️❌:       n1 -= len1;
            out_pos += class_left;
            ...
            // RDKit✔️❌:       if (n1 == 0) {
            // RDKit✔️❌:         if (ptr != s2) {
            // RDKit✔️❌:           memmove(ptr, s2, n2 * sizeof(int));
            // RDKit✔️❌:         }
            // RDKit✔️❌:         return result;
            // RDKit✔️❌:       }
            if remaining_left == 0 {
                output[out_pos..out_pos + remaining_right]
                    .copy_from_slice(&right_source[right_pos..right_pos + remaining_right]);
                return result;
            }
            // RDKit✔️❌:       s1 += len1;
            left_pos += class_left;
            // RDKit✔️❌:       memmove(ptr, s2, len2 * sizeof(int));
            output[out_pos..out_pos + class_right]
                .copy_from_slice(&right_source[right_pos..right_pos + class_right]);
            // RDKit✔️❌:       ptr += len2;
            // RDKit✔️❌:       n2 -= len2;
            out_pos += class_right;
            remaining_right -= class_right;
            // RDKit✔️❌:       if (n2 == 0) {
            // RDKit✔️❌:         memmove(ptr, s1, n1 * sizeof(int));
            // RDKit✔️❌:         return result;
            // RDKit✔️❌:       }
            if remaining_right == 0 {
                output[out_pos..out_pos + remaining_left]
                    .copy_from_slice(&left_source[left_pos..left_pos + remaining_left]);
                return result;
            }
            // RDKit✔️❌:       s2 += len2;
            right_pos += class_right;
            // RDKit✔️❌:     } else if (stat < 0 && len1 > 0) {
            ...
            // RDKit✔️❌:       s1 += len1;
            left_pos += class_left;
            // RDKit✔️❌:     } else if (stat > 0 && len2 > 0) /* stat > 0 */ {
        } else {
            // RDKit✔️❌:       memmove(ptr, s2, len2 * sizeof(int));
            output[out_pos..out_pos + class_right]
                .copy_from_slice(&right_source[right_pos..right_pos + class_right]);
...
fn compare_canon_atoms_for_kekulize(
    ...
    compare_canon_atom_base_for_kekulize(atoms, left, right, flags).then_with(|| {
        for idx in 0..atoms[left].bonds.len().min(atoms[right].bonds.len()) {
            let cmp = compare_canon_bond_holder(
                &atoms[left].bonds[idx],
                &atoms[right].bonds[idx],
                &ranks,
            );
...
fn compare_special_chirality_atoms_for_kekulize(
    ...
    // RDKit❗✔️:     if (dp_atomsInPlay && !((*dp_atomsInPlay)[i] || (*dp_atomsInPlay)[j])) {
    // RDKit❗✔️:       return 0;
    // RDKit❗✔️:     }
    ...
    // RDKit✔️✔️:     }
    let swaps_left = update_atom_neighbor_num_swaps_for_kekulize(atoms, left);
    let swaps_right = update_atom_neighbor_num_swaps_for_kekulize(atoms, right);
...
fn compare_canon_atom_base_for_kekulize(
    ...
    // RDKit❗✔️:   if (df_useNonStereoRanks) {
    // RDKit❗✔️:     int rankingNumber_i = 0;
    // RDKit❗✔️:     int rankingNumber_j = 0;
    ...
    cmp = left_atom.formal_charge.cmp(&right_atom.formal_charge);
    if cmp != Ordering::Equal {
        return cmp;
    }
...
fn compare_ring_atoms_concerning_num_neighbors_for_kekulize(
    ...
    // RDKit❗✔️:     if (!ringInfo->isInitialized() ||
...
mod tests {
    ...
    fn rank_fragment_atoms_accepts_symbol_overrides() {
        ...
        let ranks = rank_fragment_atoms(&molecule, scope, CanonicalRankOptions::kekulize_default())
            .unwrap();
