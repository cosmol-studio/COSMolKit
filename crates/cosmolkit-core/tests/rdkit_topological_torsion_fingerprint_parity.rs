...
use cosmolkit_core::fingerprint::{
    AdditionalOutput, FingerprintFuncArguments, TopologicalTorsionArguments, TypedFingerprintGenerator,
    generatorFromJSON, generatorToJSON, getTopologicalTorsionGenerator,
};
use cosmolkit_core::{
    AtomId, Molecule, TopologicalTorsionLegacyKind, TopologicalTorsionLegacyParams, TopologicalTorsionLegacyResult,
    get_atom_code, get_topological_torsion_code, topological_torsion_legacy_fingerprint,
};
use rayon::prelude::*;
...
fn profile_generator(profile: &GoldenProfile) -> cosmolkit_core::TopologicalTorsionFingerprintGenerator {
    ...
    arguments.fingerprint_arguments.d_num_bits_per_feature = value_u32(parameters, "numBitsPerFeature", 1);
    getTopologicalTorsionGenerator(&arguments, None, true).expect("valid oracle generator")
}

fn profile_call_arguments(molecule: &Molecule, profile: &GoldenProfile, provenance: bool) -> FingerprintFuncArguments {
    let parameters = &profile.parameters;
    let mut arguments = FingerprintFuncArguments {
        from_atoms: (parameters.get("fromAtoms").and_then(Value::as_str) == Some("first") && molecule.num_atoms() > 0)
            .then(|| vec![0]),
        ignore_atoms: (parameters.get("ignoreAtoms").and_then(Value::as_str) == Some("first")
            && molecule.num_atoms() > 0)
            .then(|| vec![0]),
        custom_atom_invariants: (parameters.get("customAtomInvariants").and_then(Value::as_str)
            == Some("index_plus_17"))
        .then(|| (0..molecule.num_atoms()).map(|index| index as u32 + 17).collect()),
        ..Default::default()
    };
    if provenance {
...
fn assert_modern_profile(row: usize, name: &str, molecule: &Molecule, expected: &GoldenProfile) {
    ...
    let actual_json: Value = serde_json::from_str(&generatorToJSON(&TypedFingerprintGenerator::TopologicalTorsion(
        generator.clone(),
    )))
    ...
    assert_eq!(count.nonzero_elements(), &expected_counts(&expected.count), "{context}");
    ...
    assert_eq!(
        bit.on_bits().into_iter().map(|bit| bit as u64).collect::<Vec<_>>(),
    ...
    if let Some(outputs) = &expected.additional_output {
        assert_output(
            ...
            sparse_count_args.additional_output.as_ref().expect("output"),
    ...
    let bulk_sparse_count = generator
        ...
        .flatten()
        .expect("bulk item");
    assert_eq!(bulk_sparse_count.size(), expected.bulk.sparse_count.size, "{context}");
    assert_eq!(
    ...
    let bulk_sparse_bit = generator
        ...
        .flatten()
        .expect("bulk item");
    assert_eq!(bulk_sparse_bit.size(), expected.bulk.sparse_bit.size, "{context}");
    assert_eq!(
        bulk_sparse_bit.on_bits().iter().copied().collect::<Vec<_>>(),
    ...
    let bulk_bit = generator
        ...
        .flatten()
        .expect("bulk item");
    assert_eq!(bulk_bit.n_bits() as u64, expected.bulk.bit.size, "{context}");
    assert_eq!(
        bulk_bit.on_bits().into_iter().map(|bit| bit as u64).collect::<Vec<_>>(),
...
fn assert_legacy(row: usize, molecule: &Molecule, expected: &GoldenLegacy) {
    ...
    let cases = [
        ...
        (
            "unfolded_custom",
            TopologicalTorsionLegacyParams {
                atom_invariants: Some((0..molecule.num_atoms()).map(|index| index as u32 + 17).collect()),
                ..Default::default()
            },
    ...
    for (name, params, golden) in cases {
        ...
        assert_eq!(actual.nonzero_elements(), &expected_counts(golden), "{context} {name}");
    }
    for (bits_per_entry, golden) in &expected.bit_vectors {
        ...
        assert_eq!(
            actual.on_bits().into_iter().map(|bit| bit as u64).collect::<Vec<_>>(),
            golden.on_bits,
...
fn assert_helpers(row: usize, molecule: &Molecule, expected: &GoldenHelpers) {
    ...
    let unfolded = topological_torsion_legacy_fingerprint(molecule, &TopologicalTorsionLegacyParams::default())
        .expect("unfolded ids");
...
fn assert_error_profile(row: usize, molecule: &Molecule, errors: &BTreeMap<String, GoldenError>) {
    ...
    assert_eq!(errors["invalid_json"].error_type.as_deref(), Some("RuntimeError"));
    assert!(
        errors["invalid_json"]
            .message
            .as_deref()
    ...
    assert_eq!(errors["short_path"].error_type.as_deref(), Some("IndexError"));
    assert_eq!(errors["short_path"].message.as_deref(), Some("list index out of range"));
    ...
    assert!(generatorFromJSON("not json").is_err(), "row {row}: invalid JSON");
...
fn assert_active_profile_exactly(expected_profiles: &[&str]) {
    ...
    let mut outcomes = golden_reader()
        ...
        .map(|(row, content)| {
            std::panic::catch_unwind(std::panic::AssertUnwindSafe(|| {
                let content = content.unwrap_or_else(|error| panic!("failed to read golden line {}: {error}", row + 1));
                let record: GoldenRecord = serde_json::from_str(&content)
                    .unwrap_or_else(|error| panic!("failed to parse golden line {}: {error}", row + 1));
                let smiles = corpus.get(row).unwrap_or_else(|| panic!("golden has extra row {row}"));
                assert_eq!(record.row, row, "row index changed");
                assert_eq!(record.smiles, *smiles, "row {row}: SMILES changed");
                if !record.rdkit_ok {
                    assert!(record.error.is_some(), "row {row}: missing RDKit parse error");
                    assert!(record.profiles.is_empty(), "row {row}: failed row has profiles");
                    assert!(Molecule::from_smiles(smiles).is_err(), "row {row}: parse mismatch");
                    return;
                }
                assert!(record.error.is_none(), "row {row}: success row has error");
                assert_eq!(
                    record.profiles.keys().map(String::as_str).collect::<Vec<_>>(),
                    expected_profiles,
                    "row {row}: profile set changed"
                );
                let molecule =
                    Molecule::from_smiles(smiles).unwrap_or_else(|error| panic!("row {row} parse failed: {error}"));
    ...
    assert_eq!(outcomes.len(), corpus.len(), "golden must consume every corpus row");
    assert!(
        outcomes
            .iter()
            .enumerate()
