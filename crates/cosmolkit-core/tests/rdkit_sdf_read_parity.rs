...
use cosmolkit_core::{
    BatchErrorMode, BatchRecord, BatchRecordError, BondDirection, BondOrder, BondStereo, ChiralTag,
    MoleculeBatch, SmilesWriteParams,
    io::sdf::{SdfReadParams, read_sdf_from_str_with_params},
};
...
fn ensure_golden_exists() {
    ...
    assert!(
        ...
        "missing RDKit SDF read golden: {}. Generate it before running tests:\n\
         uv sync --group dev && .venv/bin/python tests/scripts/gen_rdkit_sdf_read_golden.py --input tests/smiles.smi --output tests/golden/sdf_read.jsonl",
        path.display()
    );
}
...
fn parsed_record(record: &SdfReadRecord, row_idx: usize) -> cosmolkit_core::io::sdf::SdfRecord {
    ...
    // removeHs=False) for the CTAB payload inside each SDF record, so parity
    // must bind the same reader parameters.
...
fn sdf_read_to_smiles_matches_rdkit_canonical_and_noncanonical() {
    ...
    for (row_idx, (record, noncanonical)) in records.iter().zip(noncanonical).enumerate() {
        if !record.rdkit_ok {
            continue;
        }
        let expected = record
            ...
            .as_ref()
            .expect("rdkit_ok row should have SMILES output");
        if let Some(canonical_values) = canonical.as_ref() {
            assert_eq!(
                &canonical_values[row_idx],
                &expected.canonical,
...
fn batch_from_sdf_records(records: &[SdfReadRecord]) -> MoleculeBatch {
    MoleculeBatch::new(
        records
            ...
            .enumerate()
            .map(|(index, record)| {
                let sdf = record.sdf.as_deref().unwrap_or_default();
                match read_sdf_from_str_with_params(
                    sdf,
                    SdfReadParams {
                        sanitize: true,
                        remove_hs: false,
