...
use cosmolkit_core::{
    BondDirection, ChiralTag, Molecule, SmartsParseParams, SmilesWriteParams, SubstructMatchParams,
    get_atom_smarts, get_bond_smarts, mol_fragment_to_cx_smarts, mol_fragment_to_smarts,
    mol_from_smarts, mol_to_cx_smarts, mol_to_smarts, try_get_substruct_matches_with_params,
};
...
fn smarts_fragment_and_cx_apis_compose_with_source_corpus() {
    for record in records().filter(|record| record.parse_ok) {
        ...
        let fragment =
            mol_fragment_to_smarts(&first, &SmilesWriteParams::default(), &atoms, bonds_to_use)
                .unwrap();
        assert_eq!(
        ...
        let fragment_cx =
            mol_fragment_to_cx_smarts(&first, &SmilesWriteParams::default(), &atoms, bonds_to_use)
                .unwrap();
        mol_from_smarts(&cx, &SmartsParseParams::default())
            .unwrap_or_else(|error| panic!("{} CXSMARTS {cx:?}: {error}", record.case_id));
        mol_from_smarts(&fragment_cx, &SmartsParseParams::default()).unwrap_or_else(|error| {
            panic!(
...
fn smarts_directional_bond_state_matches_rdkit_grammar_paths() {
    for (smarts, bond_index, expected) in [
        ...
        let molecule = mol_from_smarts(smarts, &SmartsParseParams::default())
            .unwrap_or_else(|error| panic!("{smarts}: {error}"));
        assert_eq!(
            molecule.bonds()[bond_index].direction(),
            expected,
            "{smarts}"
        );
    }
