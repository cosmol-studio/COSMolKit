...
use cosmolkit_core::{
    RingFindingError, RingInfo, ValenceAssignment, ValenceError, ValenceModel,
    fast_find_rings_from_parts,
};
use cosmolkit_model::{
    AdjacencyList, Atom, AtomId, Bond, BondId, StereoGroup, StereoGroupKind, TopologyBlock,
};
use cosmolkit_types::{BondOrder, BondStereo, ChiralTag};

#[derive(Debug, thiserror::Error)]
pub(crate) enum CanonicalRankError {
...
pub(crate) fn rank_component_atoms(
    ...
    let view = CanonRankReadView::from_topology(&fragment, &fragment.stereo_groups)?;
    let local_ranks =
