...
impl SsMatcher {
    ...
    pub fn try_new(pattern: &str) -> Result<Self, FingerprintError> {
        ...
        let matcher = parse_smarts(pattern, &SmartsParseParams::default()).map_err(|error| {
...
impl From<SubgraphPathError> for FingerprintError {
    fn from(error: SubgraphPathError) -> Self {
        match error {
            SubgraphPathError::InvalidArguments { reason } => Self::InvalidArguments { reason },
        }
    }
}
...
mod tests {
    ...
    fn layered_preparation_builds_source_ordered_atom_bond_and_query_caches() {
        ...
        // Query graphs are owned by the search layer and are intentionally
        // excluded from the concrete-molecule fingerprint preparation path.
    }
    ...
    fn layered_query_parity_matches_source_masks_and_aromaticity() {
        ...
        let assert_case = |case: &LayeredQueryCase| {
            if case.notation == "smarts" {
                // Layered fingerprints currently accept concrete molecules;
                // SMARTS remains a QueryGraph-owned search value.
                return;
            }
    ...
    #[test]
    fn layered_atom_type_aromaticity_encoders_use_query_suppression_masks() {
        ...
        let mut builder = Molecule::builder();
        let aromatic = builder.add_atom(AtomSpec::new(Element::C).with_aromatic(true));
        let aliphatic = builder.add_atom(AtomSpec::new(Element::C));
        builder
            .add_bond(BondSpec::new(aromatic, aliphatic, BondOrder::Single))
            .expect("query-aware aromaticity bond");
        let molecule = builder.build().expect("aromaticity fixture");
        let prepared = prepare_layered_fingerprint(&molecule, 1, 7, 2048, None, None)
