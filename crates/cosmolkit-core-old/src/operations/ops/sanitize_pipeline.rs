...
fn sanitize_recompute_property_cache(
    ...
    let valence = crate::chemistry::sanitize::assign_property_cache_for_topology(topology, strict)
...
pub(super) fn run_sanitize_pipeline_on_topology(
    ...
    // RDKit✔️✔️:   }
    let _valence = sanitize_properties_stage(parts, topology, property_cache_strict, operation)?;
    ...
    if ops.contains(crate::SanitizeOps::PROPERTIES) {
        sanitize_properties_stage(parts, topology, true, operation)?;
    }
