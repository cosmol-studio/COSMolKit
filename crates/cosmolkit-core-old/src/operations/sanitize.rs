...
/// Recomputes the RDKit-like property-cache values for detached topology data.
///
/// Cache installation remains a runtime responsibility. This function only
/// evaluates the source-backed valence assignment against the supplied model
/// value and therefore does not require a live `Molecule` or `OpParts`.
pub fn sanitize_properties_for_topology(
    topology: &cosmolkit_model::TopologyBlock,
    strict: bool,
) -> Result<crate::ValenceAssignment, crate::ValenceError> {
    crate::chemistry::sanitize::assign_property_cache_for_topology(topology, strict)
}
...
mod tests {
    use super::{
        SanitizeOps, SanitizeStep, detect_chemistry_problems, sanitize_properties_for_topology,
    };
    ...
    #[test]
    fn detached_property_cache_matches_molecule_read_boundary() {
        let molecule = Molecule::from_smiles_with_sanitize("CCO", false).unwrap();
        let expected = crate::valence::assign_valence_with_options_for_topology(
            molecule.topology_block(),
            crate::ValenceModel::RdkitLike,
            false,
        )
        .unwrap();
        let actual = sanitize_properties_for_topology(molecule.topology_block(), false).unwrap();
