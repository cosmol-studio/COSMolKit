...
use crate::{
    ...
    BondStereo, ChiralTag, Molecule, NeighborRef, SubstanceGroup, ValenceAssignment, ValenceModel,
    molecule::TopologyBlock, ops::MoleculeReadParts,
};
...
#[derive(Debug, Clone, PartialEq, Eq, Default)]
#[allow(dead_code)]
pub struct AddHsAssignment {
    pub hydrogens_to_add: Vec<AddHydrogen>,
    pub atoms_to_update_property_cache: Vec<AtomId>,
    pub valence_before_add_hs: Option<crate::ValenceAssignment>,
    pub atom_explicit_hydrogen_updates: Vec<AtomExplicitHydrogenUpdate>,
    pub atom_pdb_residue_info_updates: Vec<AtomPdbResidueInfoUpdate>,
    pub clear_isotopic_hydrogen_properties: Vec<AtomId>,
    pub clear_computed_properties: bool,
    pub add_terminal_coordinates: bool,
}

#[derive(Debug, Clone, PartialEq, Eq)]
#[allow(dead_code)]
pub struct AddHydrogen {
    pub heavy_atom: AtomId,
    pub isotope: Option<u16>,
    pub is_implicit: bool,
    pub props: BTreeMap<String, String>,
    pub pdb_residue_info: Option<AtomPdbResidueInfo>,
}

#[derive(Debug, Clone, PartialEq, Eq, Default)]
#[allow(dead_code)]
pub struct RemoveHsAssignment {
    pub atoms_to_remove: Vec<AtomId>,
    pub atom_valence_cache_updates: Vec<AtomValenceCacheUpdate>,
    pub atom_explicit_hydrogen_updates: Vec<AtomExplicitHydrogenUpdate>,
    pub atom_chirality_inversions: Vec<AtomId>,
    pub atom_property_updates: Vec<AtomPropertyUpdate>,
    pub bond_direction_updates: Vec<BondDirectionUpdate>,
    pub bond_stereo_updates: Vec<BondStereoUpdate>,
    pub bond_stereo_atom_replacements: Vec<BondStereoAtomReplacement>,
    pub sgroup_updates: Vec<SGroupRemoveHsUpdate>,
    pub sanitize_after_removal: bool,
    pub clear_computed_properties: bool,
}

#[derive(Debug, Clone, PartialEq, Eq)]
#[allow(dead_code)]
pub struct AtomValenceCacheUpdate {
    pub atom: AtomId,
...
#[derive(Debug, Clone, PartialEq, Eq)]
#[allow(dead_code)]
pub struct AtomExplicitHydrogenUpdate {
    pub atom: AtomId,
...
#[derive(Debug, Clone, PartialEq, Eq)]
#[allow(dead_code)]
pub struct AtomPdbResidueInfoUpdate {
    pub atom: AtomId,
    pub pdb_residue_info: AtomPdbResidueInfo,
}

#[derive(Debug, Clone, PartialEq, Eq)]
#[allow(dead_code)]
pub enum AtomPropertyUpdate {
...
#[derive(Debug, Clone, PartialEq, Eq)]
#[allow(dead_code)]
pub struct BondDirectionUpdate {
    pub bond: BondId,
...
#[derive(Debug, Clone, PartialEq, Eq)]
#[allow(dead_code)]
pub struct BondStereoUpdate {
    pub bond: BondId,
    pub stereo: BondStereo,
    pub stereo_atoms: Option<[AtomId; 2]>,
}

#[derive(Debug, Clone, PartialEq, Eq)]
#[allow(dead_code)]
pub struct BondStereoAtomReplacement {
    pub bond: BondId,
    pub old_atom: AtomId,
    pub new_atom: AtomId,
}
...
pub(crate) fn add_hs_assignment(
    ...
    let valence = match read_parts.derived_cache().valence.as_ref() {
        ...
        None => {
            computed_valence = crate::valence::assign_valence_with_options_for_topology(
                read_parts.topology(),
    ...
    add_hs_assignment_for_topology(read_parts.topology(), valence, params)
}

/// Plans AddHs over a detached topology and a detached valence assignment.
///
/// The returned records are consumed by the parent runtime, which alone may
/// apply them to a live molecule and update derived state.
/// Plans hydrogen addition from detached model values.
///
/// The planner does not mutate the supplied topology or any runtime cache;
/// the parent runtime applies the returned assignment transactionally.
pub fn add_hs_assignment_for_topology(
    ...
    {
        return Err(crate::ValenceError::UnsupportedBranch {
            reason: "AddHs detached valence assignment length does not match the atom count",
        }
        .into());
    }
    if let Some(only_on_atoms) = &params.only_on_atoms {
        for atom in only_on_atoms {
            if topology.atoms.get(atom.index()).is_none() {
    ...
    for atom in &topology.atoms {
        ...
        if params.skip_queries && is_add_hs_query_atom(topology, atom.id())? {
    ...
    if params.add_residue_info {
        add_hs_residue_info_assignment(topology, &mut assignment);
    }
...
fn add_hs_residue_info_assignment(topology: &TopologyBlock, assignment: &mut AddHsAssignment) {
    ...
    let adjacency = AdjacencyList::from_topology(topology.atoms.len(), &topology.bonds);
    ...
    for atom in &topology.atoms {
        ...
        for neighbor in adjacency.neighbors_of(atom.id().index()) {
            let neighbor_atom = AtomId::new(neighbor.atom_index);
            let Some(neighbor_data) = topology.atoms.get(neighbor_atom.index()) else {
                continue;
            };
...
fn is_add_hs_query_atom(topology: &TopologyBlock, atom: AtomId) -> Result<bool, AddHydrogensError> {
    ...
    // RDKit✔️✔️: }
    let atom_data = topology
        .atoms
        .get(atom.index())
        .ok_or(AddHydrogensError::UnsupportedAtom {
            atom,
            atomic_number: 0,
        })?;
    ...
    for neighbor in topology.adjacency.neighbors_of(atom.index()) {
        let _bond =
            topology
                ...
                .ok_or(AddHydrogensError::ProtocolDebt {
...
pub(crate) fn remove_hs_assignment(
    ...
    remove_hs_assignment_for_topology(read_parts.topology(), params, sanitize, true)
}

/// Selects removable hydrogen atoms from detached topology data.
///
/// This boundary covers candidate selection and SGroup filtering only. It
/// returns stable atom IDs for the runtime to apply; it never mutates or
/// constructs a live molecule.
pub fn remove_hs_candidates_for_topology(
    topology: &TopologyBlock,
    params: &RemoveHsParams,
) -> Result<Vec<AtomId>, RemoveHydrogensError> {
    let context = RemoveHsPredicateContext::new(topology);
    let mut atoms_to_remove = topology
        .atoms
        .iter()
        .filter_map(|atom| {
            Some(should_remove_h(&context, atom.id(), params).map(|remove| (atom.id(), remove)))
        })
        .collect::<Result<Vec<_>, _>>()?
        .into_iter()
        .filter_map(|(atom, remove)| remove.then_some(atom))
        .collect::<Vec<_>>();

    if params.remove_in_sgroups {
        let original = atoms_to_remove.clone();
        atoms_to_remove.retain(|atom| {
            topology.substance_groups.iter().all(|group| {
                let members = group.atoms();
                members.is_empty()
                    || members
                        .iter()
                        .any(|member| member != atom && !original.contains(member))
            })
        });
    }
    Ok(atoms_to_remove)
...
/// Plans hydrogen removal from detached model values.
///
/// The returned assignment contains all atom, bond, stereo, and SGroup edits
/// required by the source operation. Applying those edits and installing
/// derived state remains the runtime's responsibility.
pub fn remove_hs_assignment_for_topology(
    topology: &TopologyBlock,
    params: &RemoveHsParams,
    sanitize: bool,
    ...
    if params.remove_and_track_isotopes && allow_isotope_prepass {
        ...
        if need_remove_hs {
            ...
            let first_pass =
                remove_hs_assignment_for_topology(topology, &first_pass_params, false, false)?;
            let (first_pass_topology, first_pass_mapping) =
                remove_hs_recursive_planning_topology(topology, &first_pass)?;
            let second_pass =
                remove_hs_assignment_for_topology(&first_pass_topology, params, sanitize, false)?;
    ...
    let isotopic_hydrogens = if params.remove_and_track_isotopes {
        tracked_isotopic_hydrogens(topology)?
    } else {
        Vec::new()
    };
    ...
    if params.remove_in_sgroups {
        filter_sgroup_emptying_hydrogens_for_assignment(&context, &mut atoms_to_remove);
    }
    ...
    let (atom_valence_cache_updates, cached_total_valence) =
        remove_hs_property_cache_update(topology)?;
    ...
    if sanitize && params.remove_nonimplicit && !topology.atoms.is_empty() {
...
fn remove_hs_recursive_planning_topology(
    source: &TopologyBlock,
    assignment: &RemoveHsAssignment,
) -> Result<(TopologyBlock, crate::molecule::TopologyMapping), RemoveHydrogensError> {
    ...
    topology
        ...
        .map_err(|_| RemoveHydrogensError::ProtocolDebt {
            ...
            reason: "recursive prepass produced an invalid detached topology",
        })?;
    Ok((topology, mapping))
...
impl<'a> RemoveHsPredicateContext<'a> {
    fn new(topology: &'a TopologyBlock) -> Self {
        let adjacency = AdjacencyList::from_topology(topology.atoms.len(), &topology.bonds);
        Self {
            topology,
            adjacency,
    ...
    fn atom(&self, atom: AtomId) -> Result<&'a Atom, RemoveHydrogensError> {
        let atom_count = self.num_atoms();
        self.atoms()
            .get(atom.index())
            .ok_or(RemoveHydrogensError::AtomOutOfRange { atom, atom_count })
    }

    fn bond(&self, bond: BondId) -> Result<&'a Bond, RemoveHydrogensError> {
        let bond_count = self.num_bonds();
        self.bonds()
            .get(bond.index())
            .ok_or(RemoveHydrogensError::BondOutOfRange { bond, bond_count })
    }
    ...
    fn substance_groups(&self) -> &'a [SubstanceGroup] {
...
fn filter_sgroup_emptying_hydrogens_for_assignment(
    ...
    atoms_to_remove.retain(|atom| {
        context.substance_groups().iter().all(|sgroup| {
...
fn remove_hs_property_cache_update(
    ...
    // RDKit✔️❌:     atom->updatePropertyCache(false);
    // RDKit✔️❌:   }
    let assignment = crate::valence::assign_valence_with_options_for_topology(
            topology,
            ValenceModel::RdkitLike,
            false,
        )
    ...
    let mut updates = Vec::with_capacity(topology.atoms.len());
    let mut total_valence = Vec::with_capacity(topology.atoms.len());
...
fn tracked_isotopic_hydrogens(
    ...
    crate::source_port_helpers::rdkit_get_iso_map(
        topology.bonds.iter().map(|bond| (bond.begin(), bond.end())),
        |atom_id| {
            topology
                .atoms
                .get(atom_id.index())
...
fn should_remove_h(
    ...
    if params.remove_in_sgroups {
        for substance_group in context.substance_groups() {
...
fn should_increment_explicit_h_count(
    ...
    Ok(((heavy_atom_data.atomic_number() == 7
        ...
        || may_need_extra_h(context, heavy_atom, cached_total_valence)?)
...
mod tests {
    ...
    fn shared_get_iso_map_remove_hs_adapter_preserves_atom_range_error() {
        ...
        let error = tracked_isotopic_hydrogens(molecule.topology_block()).unwrap_err();
    ...
    fn should_remove_h_matches_basic_rdkit_guards() {
        ...
        // Query state belongs to QueryGraph; concrete topology atoms do not
        // carry a query payload and therefore follow the ordinary H policy.
    ...
    #[test]
    fn detached_add_hs_planner_matches_runtime_read_boundary() {
        let molecule = Molecule::from_smiles_with_sanitize("CCO", false).unwrap();
        let valence = crate::valence::assign_valence_with_options_for_topology(
        ...
        let runtime_assignment = add_hs_assignment(read(&molecule), &params).unwrap();
        let detached_assignment =
            add_hs_assignment_for_topology(molecule.topology_block(), &valence, &params).unwrap();

        assert_eq!(runtime_assignment, detached_assignment);
        assert_eq!(
            detached_assignment.hydrogens_to_add.len(),
            6,
            "RDKit AddHs for CCO materializes six hydrogens"
        );
    }

    #[test]
    fn detached_remove_hs_candidates_match_runtime_read_boundary() {
        let mut builder = MoleculeBuilder::new();
        let carbon = builder.add_atom(AtomSpec::new(Element::C).with_no_implicit(true));
        let attached_hydrogen = builder.add_atom(AtomSpec::new(Element::H));
        let isolated_hydrogen = builder.add_atom(AtomSpec::new(Element::H));
        builder
            .add_bond(BondSpec::new(carbon, attached_hydrogen, BondOrder::Single))
            .unwrap();
        let molecule = builder.build().unwrap();

        let default_params = RemoveHsParams::default();
        let runtime_default = remove_hs_assignment(read(&molecule), &default_params, false)
            .unwrap()
            .atoms_to_remove;
        let detached_default =
            remove_hs_candidates_for_topology(molecule.topology_block(), &default_params).unwrap();
        assert_eq!(detached_default, runtime_default);
        assert_eq!(detached_default, vec![attached_hydrogen]);

        let degree_zero_params = RemoveHsParams {
            remove_degree_zero: true,
            ..default_params
        };
        let runtime_degree_zero = remove_hs_assignment(read(&molecule), &degree_zero_params, false)
            .unwrap()
            .atoms_to_remove;
        let detached_degree_zero =
            remove_hs_candidates_for_topology(molecule.topology_block(), &degree_zero_params)
                .unwrap();
        assert_eq!(detached_degree_zero, runtime_degree_zero);
        assert_eq!(
            detached_degree_zero,
            vec![attached_hydrogen, isolated_hydrogen]
        );
    }

    #[test]
    fn detached_remove_hs_assignment_matches_runtime_read_boundary() {
        let mut builder = MoleculeBuilder::new();
        let carbon = builder.add_atom(
            AtomSpec::new(Element::C)
                .with_no_implicit(true)
                .with_tracked_isotopic_hydrogens(vec![9]),
        );
        let protium = builder.add_atom(AtomSpec::new(Element::H));
        let deuterium = builder.add_atom(AtomSpec::new(Element::H).with_isotope(2));
        builder
        ...
        let runtime_assignment = remove_hs_assignment(read(&molecule), &params, true).unwrap();
        let detached_assignment =
            remove_hs_assignment_for_topology(molecule.topology_block(), &params, true, true)
                .unwrap();
        assert_eq!(detached_assignment, runtime_assignment);
    }
    ...
    fn should(molecule: &Molecule, atom: AtomId, params: &RemoveHsParams) -> bool {
        let context = RemoveHsPredicateContext::new(molecule.topology_block());
