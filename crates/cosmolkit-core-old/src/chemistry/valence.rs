...
pub fn assign_radicals(molecule: &Molecule) -> Result<Vec<u8>, ValenceError> {
    assign_radicals_for_topology(molecule.topology_block())
}

/// Computes radical electrons from a detached topology value.
///
/// The topology is an owned model value extracted by the runtime. This
/// function has no access to live molecule state, derived-cache authority, or
/// operation lifecycle; callers decide whether and how to install its result.
pub fn assign_radicals_for_topology(topology: &TopologyBlock) -> Result<Vec<u8>, ValenceError> {
    assign_radicals_from_parts(&topology.atoms, &topology.bonds, &topology.adjacency)
}
...
/// Assigns valence for a detached topology using RDKit's strict default.
///
/// This is the detached counterpart to [`assign_valence`]. The calculation is
/// entirely value-based; no molecule property cache is read or updated.
pub fn assign_valence_for_topology(
    topology: &TopologyBlock,
    model: ValenceModel,
) -> Result<ValenceAssignment, ValenceError> {
    assign_valence_with_options_for_topology(topology, model, true)
}
...
pub(crate) fn total_num_hydrogens(
    ...
    let Some(valence) = molecule.derived_cache().valence.as_ref() else {
        if atom.no_implicit() {
            let empty = ValenceAssignment {
                explicit_valence: Vec::new(),
                implicit_hydrogens: Vec::new(),
            };
            return total_num_hydrogens_for_topology(topology, &empty, atom_id, include_neighbors);
        }
        return Err(ValenceError::ImplicitValenceCacheNotInitialized { atom: atom_id });
    };
    total_num_hydrogens_for_topology(topology, valence, atom_id, include_neighbors)
}

/// Returns total hydrogens for one atom using an explicit detached valence
/// assignment. This preserves the source distinction between cached runtime
/// state and a freshly computed assignment while keeping the algorithm
/// independent of `Molecule` and its derived cache.
pub fn total_num_hydrogens_for_topology(
    topology: &TopologyBlock,
        ...
        result += topology
            ...
            .filter(|neighbor| {
                topology
                    ...
                    .is_some_and(|atom| atom.atomic_number() == 1)
            })
            .count() as i32;
        // RDKit✔️✔️:   }
    }
...
pub fn assign_valence_with_options(
    ...
    assign_valence_with_options_for_topology(molecule.topology_block(), model, strict)
}

/// Assigns explicit valence and implicit hydrogens for a detached topology.
///
/// This is the canonical algorithm boundary. The returned assignment is a
/// value result; it is not written into a molecule cache by this function.
...
pub fn atom_has_valence_violation(
    ...
    atom_has_valence_violation_for_topology(molecule.topology_block(), atom_id)
}

/// Checks one atom in a detached topology for an RDKit-style valence
/// violation without reading live molecule or cache state.
...
pub(crate) fn rdkit_rb0(atomic_number: u8) -> f64 {
    cosmolkit_types::periodic_table::rb0(atomic_number)
}
...
pub(crate) fn rdkit_atomic_mass(atomic_number: u8, isotope: Option<u16>) -> f64 {
    ...
    let atomic_weight = cosmolkit_types::periodic_table::atomic_weight(atomic_number);
...
/// Returns the RDKit periodic-table mass for a detached atom description.
///
/// This is a value-level chemistry primitive for domain crates; it does not
/// access a live molecule or any derived-cache state.
#[must_use]
pub fn atomic_mass(atomic_number: u8, isotope: Option<u16>) -> f64 {
    rdkit_atomic_mass(atomic_number, isotope)
}
...
pub fn rdkit_atomic_number_from_symbol(symbol: &str) -> Option<u8> {
    ...
    cosmolkit_types::periodic_table::atomic_number_from_symbol(symbol)
}
...
pub(crate) fn periodic_table_row(atomic_number: u8) -> Option<u8> {
    cosmolkit_types::periodic_table::period(atomic_number)
}
...
/// Returns the mass of the most common isotope for a detached atom.
pub fn most_common_isotope_mass(atomic_number: u8) -> Result<f64, ValenceError> {
    rdkit_most_common_isotope_mass(atomic_number)
}
...
fn rdkit_valence_list_for_atomic_number(
    ...
    Ok(cosmolkit_types::periodic_table::valences(atomic_number))
}
...
pub fn rdkit_element_symbol(atomic_number: u8) -> Result<&'static str, ValenceError> {
    ...
    cosmolkit_types::periodic_table::symbol(atomic_number).ok_or(ValenceError::UnsupportedBranch {
...
mod tests {
    ...
    use crate::{
        ...
        assign_radicals_for_topology, assign_valence, assign_valence_with_options,
        assign_valence_with_options_for_topology, atom_has_valence_violation,
        atom_has_valence_violation_for_topology, rdkit_valence_list,
        total_num_hydrogens_for_topology,
    };

    #[test]
    fn detached_valence_boundary_matches_molecule_facade() {
        ...
        let detached =
            assign_valence_with_options_for_topology(topology, ValenceModel::RdkitLike, false)
                .unwrap();
        assert_eq!(detached.explicit_valence, vec![1, 2, 1]);
        assert_eq!(detached.implicit_hydrogens, vec![3, 2, 1]);
        assert_eq!(
            assign_valence_with_options(&molecule, ValenceModel::RdkitLike, false).unwrap(),
            detached
        );
        assert_eq!(
            assign_radicals(&molecule).unwrap(),
            assign_radicals_for_topology(topology).unwrap()
        );
        for atom_id in (0..molecule.num_atoms()).map(AtomId::new) {
            assert_eq!(
                atom_has_valence_violation(&molecule, atom_id).unwrap(),
                atom_has_valence_violation_for_topology(topology, atom_id).unwrap()
            );
        }
        assert_eq!(
            total_num_hydrogens_for_topology(&topology, &detached, AtomId::new(0), false).unwrap(),
            3
        );
        assert_eq!(
            total_num_hydrogens_for_topology(&topology, &detached, AtomId::new(1), true).unwrap(),
            2
        );
    }
    ...
    fn smarts_query_bond_valence() {
        ...
        let plain = builder
            ...
            .unwrap();
        let simple = builder
            .add_bond(BondSpec::new(atoms[2], atoms[3], BondOrder::Double))
            .unwrap();
        let complex = builder
            .add_bond(BondSpec::new(atoms[4], atoms[5], BondOrder::Double))
            .unwrap();
        let molecule = builder.build().unwrap();
    ...
    fn atom_has_valence_violation_ignores_query_atoms_and_query_bonds_like_rdkit() {
        let mut builder = MoleculeBuilder::new();
        let query = builder.add_atom(AtomSpec::new(Element::C));
        for _ in 0..5 {
            let carbon = builder.add_atom(AtomSpec::new(Element::C));
            builder
        ...
        builder
            ...
            .unwrap();
        builder
            .add_bond(BondSpec::new(middle_left, middle_right, BondOrder::Single))
            .unwrap();
        builder
