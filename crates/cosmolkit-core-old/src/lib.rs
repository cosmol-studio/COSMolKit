//! Frozen pre-redesign core.
//!
//! This crate is kept only as reference material for algorithms, parity cases,
//! and migration reading. New `cosmolkit-core` code must not depend on this
//! crate or copy its mutable molecule architecture wholesale.
//!
//! Agent guardrail: if a future task tries to call this crate from the new core
//! to save time, the agent must stop and confirm with the human author first.
//! The redesign intentionally favors a clear value-style core over preserving
//! the old parity-first structure.
